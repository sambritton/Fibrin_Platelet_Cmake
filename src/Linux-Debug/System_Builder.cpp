...
unsigned SystemBuilder::addNode(double mass, glm::dvec3 pos ) {
...
//add platelets
//all node pointers made here. We take this opportunity to
unsigned SystemBuilder::addPlt(double pltmass, glm::dvec3 pos ) {
...
std::shared_ptr<System> SystemBuilder::create() {
	...
	std::cout << "edge count post addition: " << hostWLCLenZero.size() << std::endl;

	numNodes = hostPosX.size();
	numPlts = hostPltPosX.size();
	numEdges = hostWLCEdgeLeft.size();
	std::cout << "final node count: " << numNodes << std::endl;//Plt
	...
	std::cout<< host_ptr_devNodeSystem->generalParams.dtTemp << std::endl;
	...
	host_ptr_devNodeSystem->domainParams.gridSpacing_plt_intc = 1.5 * pltRForce;
	host_ptr_devNodeSystem->domainParams.gridSpacing_net_intc = 10.0 * defaultLinkDiameter;
